    ...
    if (D < today) {
      ...
      const query = `
        ...
        SELECT
          ...
          SUM(ds.remaining) AS amount
        FROM debt_statistics ds
        JOIN latest l ON l.original_debt_id = ds.original_debt_id
        WHERE ${where.join(' AND ')}
        GROUP BY range
        ORDER BY MIN(DATEDIFF(?, ds.due_date))
      `;
      const rows = await this.debtStatisticRepository.query(query, [D, D, D, D, ...arr, D]);
      return rows.map((r: any) => ({ range: r.range, count: Number(r.count) || 0, amount: Number(r.amount) || 0 }));
    } else {
      const where: string[] = [
      ...
      const query = `
        ...
        WHERE ${where.join(' AND ')}
        GROUP BY range
        ORDER BY MIN(DATEDIFF(?, d.due_date))
      `;
      const rows = await this.debtRepository.query(query, [D, D, D, ...arr, D]);
      return rows.map((r: any) => ({ range: r.range, count: Number(r.count) || 0, amount: Number(r.amount) || 0 }));
    }
  }
  ...
  async getPayLaterDelayAsOf(params: { singleDate?: string; from?: string; to?: string; buckets: number[]; employeeCode?: string; customerCode?: string }) {
    ...
    if (D < today) {
      ...
      const query = `
        ...
        SELECT rng AS range, COUNT(*) AS count, SUM(ds.remaining) AS amount
        FROM (
      ...
      const query = `
        SELECT rng AS range, COUNT(*) AS count, SUM(d.remaining) AS amount
        FROM (
