...
export class UserStatusObserver {
  ...
  async handleUserStatusChange(event: UserStatusChangeEvent) {
    console.log(`👁️ [UserStatusObserver] User ${event.userId} status changed from ${event.oldStatus} to ${event.newStatus}`);

    // Nếu status được cập nhật thành 2 (lỗi liên kết Zalo)
    if (event.newStatus === 2) {
      console.log(`🔄 [UserStatusObserver] Requesting token refresh for user ${event.userId} due to Zalo link error`);
      
      // Gửi socket event đến frontend yêu cầu refresh token
      this.userGateway.server.to(`user_${event.userId}`).emit('force_token_refresh', {
        userId: event.userId,
      ...
      this.userGateway.server.to('admin_dashboard').emit('user_zalo_link_error', {
        userId: event.userId,
