...
export class DebtService {
  ...
  async importExcelRows(rows: any[], userId?: number) {
        ...
        try {
          if (oldDebt) {
            ...
            insertPromises.push(
              debtRepo
                .save(debt)
                .then(async (savedDebt) => {
                  // Backup trong cùng transaction chính, không tạo nested transaction
                  await backupRepo.save({
                    import_session_id,
                    original_debt_id: savedDebt.id,
                    user_id: userId,
                    original_data: {}, // hoặc {} nếu cần
                    action_type: 'CREATE',
                  });
                ...
                .catch((err) => {
                  console.error('Error creating debt and backup:', err);
                  errorResults.push({
                    row: i + 2,
                    error: err?.message || 'Unknown error',
                  });
  ...
  async getImportHistory(date?: string) {
    ...
    const sessions = await this.debtImportBackupRepo
      ...
      .leftJoinAndSelect('backup.user', 'user')
      .select([
      ...
      .groupBy(
        'backup.import_session_id, user.id, user.fullName, user.username',
      )
  ...
  async rollbackImport(sessionId: string) {
    return await this.dataSource.transaction(async (manager) => {
      ...
      for (const backup of backups) {
        try {
          if (backup.action_type === 'CREATE') {
            // Kiểm tra debt có tồn tại không (bao gồm cả soft deleted)
            const existingDebt = await debtRepo
              .createQueryBuilder('debt')
              .withDeleted() // Include soft deleted records
              .where('debt.id = :id', { id: backup.original_debt_id })
              .getOne();

            if (existingDebt) {
              // Xóa cứng debt statistics trước
              await debtStatisticRepo
                .createQueryBuilder()
                .delete()
              ...
              // Xóa cứng debt record
              await debtRepo
                .createQueryBuilder()
                .delete()
                .where('id = :id', { id: backup.original_debt_id })
                .execute();

              createdDebtIds.push(backup.original_debt_id);
              rollbackCount++;
            }
          } else if (backup.action_type === 'UPDATE') {
            // Kiểm tra debt có tồn tại không
            const existingDebt = await debtRepo.findOne({
              where: { id: backup.original_debt_id },
            });
            ...
            if (existingDebt) {
              ...
              // Khôi phục debt_statistics nếu có
      ...
      return {
        ...
        created_debt_ids: createdDebtIds, // Danh sách các debt đã được tạo và xóa cứng
